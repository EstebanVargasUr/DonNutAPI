...
class PasswordResetRequestController extends Controller
{
    ...
    public function send($email)  //this is a function to send mail 
    {
        ...
        Mail::to($email)->send(new SendMailreset($token, $this->getUser($email)));  // token is important in send mail 
    }
    ...
    public function createToken($email)  // this is a function to get your request email that there are or not to send mail
    {
        ...
        $token = Str::random(15).now()->timestamp.Str::random(15);
        $this->saveToken($token, $email);
        return $token;
    }

    public function getUser($email){
        return User::where('email', $email)->first('nombre');
    }
